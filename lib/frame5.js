...
Frame5.prototype.init = function() {
	this.app.use(function(req, res, next) {
		...
		if (_url.pathname === '/frame5') {
			build(req, res, ['events', 'socket.io', 'core', 'async', 'browser', 'nprogress', 'pace', 'config', 'stream', 'xhr', 'rpc', 'worker', 'io', 'web-sql', 'tmpl', 'init', 'require']);
...
//
Frame5.prototype.broadcast = function(method, params, cb) {
	var j = 0;
	var k = 0;
	var d = [];
...
Frame5.prototype.initSocket = function() {
	...
	io.sockets.on('connection', function(socket) {
		...
		module.broadcast = self.broadcast.bind(self);
